...
layout(local_size_x = $iDispatchX, local_size_y = $iDispatchY, local_size_z = $iDispatchZ) in;

layout(binding = 0, rgba32f) uniform writeonly image2D ChannelOut;
...
void main() {
  ...
  //pixel = vec4(pixel_coords.x / iResolution.x, pixel_coords.y / iResolution.y, 0.0, 1.0);
  //vec4 fragColor;
  //mainImage(pixel, vec2(pixel_coords));

  // output to a specific pixel in the image
  imageStore(ChannelOut, pixel_coords, pixel);
}
