  ...
  if vars.GPU_CHUNKS_INIT
    ...
    if vars.RENDER_METHOD == 8
      ...
      if vars.single_storage
        MAX_SHADER_STORAGE_BLOCK_SIZE = glGetValue(GLint, GL_MAX_SHADER_STORAGE_BLOCK_SIZE)
        MAX_GEOMETRY_OUTPUT_VERTICES = glGetValue(GLint, GL_MAX_GEOMETRY_OUTPUT_VERTICES)
        MAX_GEOMETRY_TOTAL_OUTPUT_COMPONENTS = glGetValue(GLint, GL_MAX_GEOMETRY_TOTAL_OUTPUT_COMPONENTS )
        MAX_GEOMETRY_SHADER_INVOCATIONS = glGetValue(GLint, GL_MAX_GEOMETRY_SHADER_INVOCATIONS)
...
function checkCamera()
  vars.CAMERA_UPDATED=CameraManager.OnUpdate(vars.CAMERA)
